...
\begin{proof}%FIXME(理解して書き直す)
    対偶を示す。すなわち、「$x=\xi$ が $p_{1}(x)$ および $p_{2}(x)$ の正則点であれば、$\xi$ は解 $\varphi(x)$ の正則点である」ことを示す。

    仮定より、$p_{1}(x), p_{2}(x)$ は $x=\xi$ において正則である。コーシーの存在定理によれば、係数関数が正則である領域内の任意の点において、その近傍で正則な解がただ一つ存在する。
    さらに、線形常微分方程式の解の性質として、解の冪級数展開 $\sum c_n (x-x_0)^n$ の収束半径は、中心 $x_0$ から「係数関数の特異点」までの最短距離に等しい（あるいはそれ以上である）。

    したがって、$\xi$ が係数関数の特異点でないならば、解 $\varphi(x)$ は $\xi$ を含む開円板上まで解析接続が可能であり、その領域で正則となる。
    ゆえに $\xi$ は解 $\varphi(x)$ の特異点ではあり得ない。
    対偶が示されたことにより、解の特異点は必ず係数の特異点のいずれかに一致する。
\end{proof}
